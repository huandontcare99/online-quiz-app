const questions = [
    ...
    },
    {
        ...
        source: null // Không có source cụ thể cho câu hỏi này trong file
    },
    {
    ...
    },
    {
        ...
        source: 45 // Không có source cụ thể cho câu hỏi này trong file
    }
];
...
let userAnswers = new Array(questions.length).fill(null); // Lưu trữ câu trả lời của người dùng
let score = 0;
...
function startQuiz() {
    ...
    submitBtn.style.display = 'none'; // Ẩn nút nộp bài khi mới bắt đầu
    prevBtn.disabled = true; // Vô hiệu hóa nút "Câu trước" khi ở câu đầu tiên
...
function loadQuestion() {
    ...
    question.options.forEach((option, index) => {
        ...
        optionDiv.dataset.option = option; // Lưu giá trị đáp án vào dataset

        // Nếu người dùng đã chọn đáp án cho câu này trước đó, thì highlight lại
    ...
    // Hiển thị nút "Nộp bài" khi ở câu cuối cùng
...
function selectAnswer(selectedOption) {
    ...
    document.querySelector(`.answer-option[data-option="${selectedOption}"]`).classList.add('selected');
}
...
function startTimer() {
    clearInterval(timerInterval); // Đảm bảo chỉ có một timer chạy
...
function displayResults() {
    ...
    resultDetails.innerHTML = ''; // Xóa chi tiết cũ
